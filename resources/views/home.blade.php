...
@section('content')
    <div class="w-full max-w-screen-2xl mx-auto px-4 sm:px-8 font-inria bg-white text-gray-800">
        ...
        <div class="relative w-full h-[36rem] md:h-[44rem] overflow-hidden mt-6">
            ...
            <img src="{{ asset('images/title_image.png') }}" alt="Traditional Textiles"
                class="w-full h-full object-cover object-center transition-all duration-1000 ease-in-out hero-image">

            <div class="absolute inset-0 flex flex-col items-center justify-center z-10 px-6">
                <div class="max-w-4xl text-center">
                    <h1
                        class="text-3xl md:text-5xl lg:text-6xl font-light leading-tight text-white tracking-wide hero-title transform translate-y-10 opacity-0">
                        <span class="font-serif italic">Timeless Elegance</span> in Every Thread
                    </h1>
                    <p
                        class="mt-6 text-lg md:text-xl text-white/90 max-w-2xl mx-auto font-light hero-subtitle transform translate-y-10 opacity-0">
                        From intricate patterns to handcrafted artistry, explore Indonesia's textile heritage through our
                        curated collection.
                    </p>
                </div>

                <div class="mt-10 w-full max-w-2xl hero-form transform translate-y-10 opacity-0">
                    <form id="searchForm" action="{{ route('products') }}" method="GET" class="relative px-2">
                        <div class="relative flex items-center">
                            <!-- Search input -->
                            <input type="text" name="query" id="queryInput" placeholder="Discover our collection..."
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
                @foreach ($categories as $category)
                    <a href="{{ route('products', ['category' => $category->name]) }}"
                        class="group relative block h-80 md:h-96 overflow-hidden rounded-xl shadow-md hover:shadow-xl transition-all duration-500">
                        <div class="absolute inset-0 bg-cover bg-center transition-all duration-700 group-hover:scale-105"
                            style="background-image: url('{{ asset('images/' . strtolower($category->name) . '.png') }}');">
                        </div>
                        <div class="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-transparent"></div>
                        <div class="relative h-full flex flex-col justify-end p-6 text-white">
                            <h3
                                class="text-2xl font-medium mb-1 transform group-hover:translate-y-0 translate-y-2 transition-transform duration-300">
                                {{ $category->name }}</h3>
                            <p
                                class="text-sm opacity-0 group-hover:opacity-100 transform group-hover:translate-y-0 translate-y-4 transition-all duration-300 delay-100">
                                {{ $category->description }}</p>
                            <div
                                class="w-8 h-px bg-amber-400 mt-3 mb-2 transform group-hover:scale-x-100 scale-x-0 origin-left transition-transform duration-300 delay-200">
                            </div>
                            <span
                                class="text-xs font-light opacity-0 group-hover:opacity-100 transform group-hover:translate-y-0 translate-y-4 transition-all duration-300 delay-300">
                                View Collection →
                            </span>
                        </div>
                    </a>
        ...
        <div class="bg-gray-50 md:py-24 px-4 sm:px-6 opacity-0 md:opacity-100">
            <div class="container mx-auto">
                <div class="text-center mb-16">
                    ...
                    <p class="text-gray-600 max-w-2xl mx-auto font-light">Discover the rich diversity of textile traditions
                ...
                <div class="relative w-full max-w-5xl mx-auto p-8 rounded-xl ">
                    <div class="map-container relative">
                        <div id="map-tooltip"
                            class="absolute bg-gray-900 text-white px-3 py-2 rounded text-sm pointer-events-none opacity-0 transition-opacity">
                        </div>
        ...
        <div class="relative w-full h-[32rem] md:h-[36rem] overflow-hidden group">
            <div
                class="absolute inset-0 bg-gradient-to-r from-gray-900/50 to-gray-900/30 z-0 transition-all duration-700 group-hover:opacity-90">
            </div>
            <img src="{{ asset('images/banner2.png') }}" alt="Artisan at work"
                class="w-full h-full object-cover object-center transition-all duration-1000 ease-in-out group-hover:scale-105">

            <div class="absolute inset-0 flex items-center px-8 md:px-16 lg:px-24 z-10">
                <div class="max-w-2xl">
                    <span class="text-amber-400 font-light tracking-widest text-sm mb-4 block">HERITAGE &
                        CRAFTSMANSHIP</span>
                    ...
                    <p class="text-white/90 font-light text-lg mb-8 max-w-lg">
                        Each textile carries generations of knowledge, from natural dye techniques to symbolic motifs that
                        speak of our cultural identity.
                    </p>
                    <button
                        class="px-8 py-3 bg-transparent border border-amber-400 text-amber-400 hover:bg-amber-400 hover:text-white transition-all duration-300 rounded-full font-medium tracking-wide">
        ...
        <div class="container mx-auto px-4 sm:px-6 py-16 md:py-24">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10 product-grid">
                @foreach ($products as $product)
                    <div
                        class="group relative bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-500 product-card transform hover:-translate-y-2">
                        <div class="relative overflow-hidden h-96">
                            ...
                            <div class="absolute top-4 right-4">
                                <button
                                    class="p-2 bg-white/90 rounded-full shadow-md hover:bg-amber-500 hover:text-white transition-all duration-300 opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0">
                        ...
                        <div class="p-6">
                            ...
                            <div class="flex justify-between items-center">
                                <!-- Assuming price is an array or object after decoding JSON in the controller -->
                                <span class="text-amber-600 font-medium">From IDR
                                    @if (is_array($product->price))
                                        {{ number_format($product->price[0]['price'], 0, ',', '.') }}
                                    @else
                                ...
                                <a href="{{ url('products/' . $product->id) }}"
                                    class="px-4 py-2 border border-gray-300 text-sm rounded-full hover:bg-amber-600 hover:text-white hover:border-amber-600 transition-all duration-300">
                                    View Details
                                </a>
                            </div>
                        </div>
            ...
            <div class="text-center mt-16">
                <a href="{{ route('products') }}">
                    <button
                        class="px-8 py-3 border border-gray-300 rounded-full hover:bg-gray-900 hover:text-white transition-all duration-300 font-medium tracking-wide">
        ...
        <div class="container mx-auto px-4 sm:px-6 py-16 md:py-24">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 md:gap-12">
                <div class="relative h-[32rem] md:h-[36rem] rounded-xl overflow-hidden shadow-lg group">
                    <img src="{{ asset('images/kain_songket_palembang.jpg') }}" alt="Artisan at work"
                        class="w-full h-full object-cover transition-transform duration-1000 ease-in-out group-hover:scale-105">
                    <div
                        class="absolute inset-0 bg-gradient-to-t from-black/60 via-black/30 to-transparent flex items-end p-8">
                        <div
                            class="text-white transform translate-y-10 group-hover:translate-y-0 transition-all duration-500">
                            ...
                            <p
                                class="text-white/90 font-light max-w-md opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-200">
                                Woven with gold and silver threads, this royal textile embodies the grandeur of Srivijaya
                                heritage.
                            </p>
                            <button
                                class="mt-4 px-6 py-2 border border-white/30 rounded-full text-sm hover:bg-white hover:text-gray-900 transition-all duration-300 opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 delay-300">
                                Learn the History →
                            </button>
                        </div>
                    </div>
                ...
                <div
                    class="relative flex flex-col justify-between bg-gray-50 rounded-xl p-8 md:p-10 h-[52rem] md:h-[36rem] overflow-hidden">
                    ...
                    <div class="relative z-10 mt-8">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div
                                class="bg-white p-6 rounded-lg shadow-sm border border-gray-100 hover:shadow-md transition-all duration-300">
            ...
            <div class="container mx-auto">
                ...
                <div class="relative">
                    ...
                    <div id="logo-container" class="overflow-hidden py-4">
                        <ul id="logos"
                            class="flex items-center justify-center [&_li]:mx-8 [&_img]:max-w-[8rem] [&_img]:opacity-70 [&_img]:hover:opacity-100 [&_img]:transition-all [&_img]:duration-300 animate-loop-scroll">
                            @for ($i = 0; $i < 8; $i++)
                                <li>
                                    <img src="{{ asset('images/logo.jpg') }}" alt="Partner Logo"
...
@section('script')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const triggerBtn = document.getElementById('triggerImageSearch');
            const imageInput = document.getElementById('imageSearchInput');
            const queryInput = document.getElementById('queryInput');
            const searchForm = document.getElementById('searchForm');

            // Klik tombol camera untuk membuka file picker
