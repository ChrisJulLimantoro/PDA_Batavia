            ...
            <nav class="mb-8 opacity-0 transform translate-y-5" id="breadcrumb-anim">
                <div class="flex items-center space-x-4">
                    <div class="text-sm flex items-center space-x-2">
                        <span class="text-gray-500 hover:text-amber-600 transition-colors">
                        ...
                        <span class="text-gray-400">/</span>
                        <span class="text-amber-600 font-medium truncate max-w-xs">{{ $product->name }}</span>
                    </div>
                </div>
            </nav>
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 p-6 md:p-10">
                    <div class="space-y-4 opacity-0" id="gallery-anim">
                        <div
                            class="relative overflow-hidden rounded-lg bg-gray-100 aspect-[4/5] flex items-center justify-center">
                            <img src="{{ $product->image }}" alt="{{ $product->name }}"
                                class="w-full h-full object-cover transition-opacity duration-500" id="main-product-image">
                            <div
                                class="absolute top-4 left-4 bg-amber-600 text-white px-3 py-1 rounded-full text-xs font-medium tracking-wider shadow-md">
                                Handcrafted
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-4 gap-3">
                            @for ($i = 0; $i < min(4, 3); $i++)
                                <button
                                    class="image-gallery-thumbnail aspect-square rounded-md overflow-hidden bg-gray-100 {{ $i === 0 ? 'active' : '' }}"
                            ...
                            @endfor
                            <div
                                class="aspect-square rounded-md bg-gray-100 flex items-center justify-center text-gray-400">
                    ...
                    <div class="opacity-0 transform translate-x-5" id="info-anim">
                        ...
                        <div class="flex flex-wrap gap-2 mb-2">
                            @foreach ($product->categories as $category)
                                <span
                                    class="bg-amber-50 text-amber-700 text-xs px-3 py-1 rounded-full border border-amber-100">
                                    {{ $category->name }}
                                </span>
                            @endforeach
                        </div>
                        ...
                        <div class="flex space-x-4">
                            <div class="flex items-center border border-gray-200 rounded-lg overflow-hidden">
                                ...
                                <button type="button" class="px-3 py-2 text-gray-500 hover:bg-gray-100 transition-colors"
                                    id="increment">
                                    <span class="hi hi-plus inline-block h-5 w-5"> + </span>
                                </button>
                            </div>
                            <button
                                class="flex-1 bg-amber-600 hover:bg-amber-700 text-white font-medium py-3 px-6 rounded-lg transition-colors duration-300 flex items-center justify-center"
                                type="button" id="order-button">
                                <span class="hi hi-shopping-cart inline-block h-5"></span>
                                Buy Now
                            </button>
                            <button
                                class="flex-1 bg-amber-600 hover:bg-amber-700 text-white font-medium py-3 px-6 rounded-lg transition-colors duration-300 flex items-center justify-center"
                                type="button" id="collab-button">
                                <span class="hi hi-shopping-cart inline-block h-5"></span>
